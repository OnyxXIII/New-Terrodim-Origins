scale set pehkui:hitbox_height 1.4
scale set pehkui:eye_height 1.4
scale set pehkui:hitbox_width 1.4
scale persist set pehkui:hitbox_height true @s
scale persist set pehkui:eye_height true @s
scale persist set pehkui:hitbox_width true @s
scale set pehkui:reach 1
scale set pehkui:step_height 0.5
scale set pehkui:motion 3.5
scale persist set pehkui:reach true @s
scale persist set pehkui:step_height true @s
scale persist set pehkui:motion true @s